...
update image i set thumbnail = replace((SELECT f.filename from files f where f.id = i.id), '.PNG', '_thumb.PNG') where i.id in (select id from files where filename like '%.PNG');
...
update image i set thumbnail = replace((SELECT f.filename from files f where f.id = i.id), '.JPG', '_thumb.JPG') where i.id in (select id from files where filename like '%.JPG');
update image i set thumbnail = replace((SELECT f.filename from files f where f.id = i.id), '.png', '_thumb.png') where i.id in (select id from files where filename like '%.png');
update image i set thumbnail = replace((SELECT f.filename from files f where f.id = i.id), '.jpeg', '_thumb.jpeg') where i.id in (select id from files where filename like '.%jpeg');
